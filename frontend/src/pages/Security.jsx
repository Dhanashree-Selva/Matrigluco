...
export default function Security() {
    ...
    const fetchUserPreferences = async () => {
        ...
        if (user) {
            setTfaActive(user.user_metadata?.two_factor_auth || false);
            setLogsActive(user.user_metadata?.security_logs !== undefined ? user.user_metadata.security_logs : true);
        }
        setFetching(false);
    };
    ...
    const togglePreference = async (key, currentValue) => {
        ...
        if (error) {
            setMessage({ text: "Failed to update preference: " + error.message, type: "error" });
            // Revert on error
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen pb-24 w-full">
            ...
            {/* Information Card */}
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mb-6 flex items-center gap-4 relative overflow-hidden">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-6">
                ...
                <form onSubmit={handleUpdatePassword} className="space-y-4">
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full border border-gray-100 bg-gray-50/50 rounded-2xl px-4 py-4 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-50/50 transition-all font-medium text-gray-700"
                                value={newPassword}
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-blue-600 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-blue-700 active:scale-[0.98] transition-all disabled:opacity-50 shadow-lg shadow-blue-100 mt-2"
                    >
                        {loading ? (
...
function SecurityOption({ label, active, onToggle }) {
    return (
        <div className="flex items-center justify-between p-4 bg-white rounded-2xl border border-gray-100 shadow-sm transition-all hover:border-blue-100">
            <span className="text-sm font-bold text-gray-700">{label}</span>
            <button
                onClick={onToggle}
                className={`w-12 h-6 rounded-full relative transition-all duration-200 outline-none focus:ring-2 focus:ring-blue-100 ${active ? 'bg-green-500 shadow-inner' : 'bg-gray-200'}`}
            >
                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-md transition-all duration-200 ${active ? 'right-1' : 'left-1'}`} />
            </button>
        </div>
    );
}
