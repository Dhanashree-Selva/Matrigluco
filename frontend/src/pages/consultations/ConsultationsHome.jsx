        ...
        <div className="min-h-screen bg-[#FDF8F8] pb-24">
            ...
            <div className="p-8 bg-white border-b border-[#F2E9E9] shadow-sm rounded-b-[40px]">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <button className="w-12 h-12 bg-[#FEE7EC] rounded-2xl flex items-center justify-center text-[#F05578] shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-[#FDF8F8] p-4 rounded-3xl border border-[#F2E9E9]">
            ...
            <div className="p-6 space-y-10">
                            ...
                            >
                                ...
                                <div className="flex-grow py-2">
                                    <div className="flex justify-between items-start mb-1">
                                        <span className={`px-2.5 py-1 rounded-full text-[9px] font-extrabold uppercase tracking-widest ${getStatusStyles(apt.status)}`}>
                                            {apt.status}
                                        </span>
                                    ...
                                    <p className="text-[10px] font-bold text-[#9E8A8C] uppercase tracking-wider">{apt.type}</p>
                                    ...
                                    <div className="mt-3 flex items-center justify-between">
                                        ...
                                        {apt.status === 'upcoming' && (
                                            <button
                                                ...
                                                className="px-4 py-1.5 bg-[#F05578] text-white text-[9px] font-extrabold rounded-full shadow-lg shadow-pink-100 hover:bg-[#E94D71] transition-colors"
                                            >
                                                JOIN
                                            </button>
                                        )}
                ...
                <section>
                    <div className="flex items-center justify-between mb-6 px-1">
                        ...
                        <button onClick={() => navigate('/doctor/list')} className="w-8 h-8 bg-white border border-[#F2E9E9] rounded-xl flex items-center justify-center text-[#2A2340]">
                    ...
                    <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 -mx-1 px-1">
                        {DOCTORS.slice(0, 4).map((doc, i) => (
                            <motion.div
                                key={doc.id}
                                whileTap={{ scale: 0.95 }}
                                className="min-w-[140px] bg-white rounded-[32px] p-4 border border-[#F2E9E9] shadow-sm text-center relative overflow-hidden"
                            >
                                <div className="absolute top-3 right-3 w-2 h-2 bg-green-500 rounded-full border-2 border-white" />
                                ...
                                <p className="text-[9px] font-bold text-[#F05578] uppercase tracking-wider mb-3 leading-tight">{doc.specialty}</p>
                                <button
                                    onClick={() => navigate(`/doctor/book/${doc.id}`)}
                                    className="w-full py-2.5 bg-[#FDF8F8] border border-[#F2E9E9] rounded-xl text-[9px] font-extrabold text-[#2A2340] hover:bg-[#FEE7EC] hover:text-[#F05578] transition-all"
                                >
                                    BOOK
                                </button>
                            </motion.div>
...
function DocCard({ icon: Icon, title, count, color, onClick }) {
    return (
        ...
        >
            <div className={`w-12 h-12 ${color} rounded-2xl flex items-center justify-center text-white mb-4 shadow-lg shadow-gray-100`}>
                <Icon size={24} />
            </div>
