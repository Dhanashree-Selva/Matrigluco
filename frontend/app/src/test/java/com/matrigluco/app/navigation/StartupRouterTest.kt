...
class StartupRouterTest {
    ...
    @Test fun `authenticated session routes to protected dashboard`() = assertEquals(RootRoute.PROTECTED, StartupRouter.route(SessionState.Authenticated(user(), false)))
    @Test fun `complete session routes to protected dashboard`() = assertEquals(RootRoute.PROTECTED, StartupRouter.route(SessionState.Authenticated(user(), true)))
