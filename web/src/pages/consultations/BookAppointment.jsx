...
import { consultationsApi, authApi } from '../../api';
...
export default function BookAppointment() {
    ...
    const handleBooking = async () => {
        ...
        try {
            // Build UTC ISO scheduled_at
            const dateStr = selectedDate.toISOString().split('T')[0];
            const [timeStr, period] = selectedSlot.split(' ');
            let [hours, minutes] = timeStr.split(':').map(Number);
            if (period === 'PM' && hours < 12) hours += 12;
            if (period === 'AM' && hours === 12) hours = 0;
            const scheduledAt = new Date(`${dateStr}T${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:00Z`).toISOString();

            await consultationsApi.createConsultation({
                provider_name: doctor.name,
                provider_specialty: doctor.specialty || "Obstetrics & Maternal-Fetal Medicine",
                consultation_type: consultationType === 'Video' ? 'video' : 'in_person',
            ...
            setError(err.response?.data?.error?.message || err.message || "Failed to book appointment.");
        } finally {
        ...
        <div className="min-h-screen bg-[#FDF8F8] relative">
            <div className="max-w-6xl mx-auto pb-[120px]">
                ...
                <div className="flex items-center px-6 py-8 bg-[#FDF8F8]">
                    <button
                        onClick={() => navigate(-1)}
                        className="w-12 h-12 bg-white rounded-md flex items-center justify-center shadow-sm border border-[#F2E9E9] hover:bg-gray-50 transition-colors"
                    >
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div className="space-y-10">
                        <section>
                            <h2 className="text-lg font-extrabold text-[#2A2340] mb-6 tracking-tight flex items-center gap-3">
                                <span className="w-8 h-8 rounded-md bg-[#FEE7EC] text-[#F05578] flex items-center justify-center text-sm">1</span>
                                Select Date
                            </h2>
                            ...
                            <div className="bg-white rounded-[40px] p-8 shadow-sm border border-[#F2E9E9]">
                                <div className="flex justify-between items-center mb-8">
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={prevMonth} className="w-10 h-10 rounded-md bg-[#FDF8F8] border border-[#F2E9E9] flex items-center justify-center text-[#9E8A8C] hover:bg-[#FEE7EC] hover:text-[#F05578] transition-all">
                                            <ChevronLeft size={20} />
                                        </button>
                                        <button onClick={nextMonth} className="w-10 h-10 rounded-md bg-[#FDF8F8] border border-[#F2E9E9] flex items-center justify-center text-[#9E8A8C] hover:bg-[#FEE7EC] hover:text-[#F05578] transition-all">
                                    ...
                                    {calendarDays.map((date, i) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`relative w-10 h-10 mx-auto flex items-center justify-center rounded-md text-sm font-bold transition-all
                                            ...
                                            >
                                                {date.getDate()}
                                                {selected && <motion.div layoutId="activeDate" className="absolute inset-0 border-2 border-[#F05578] rounded-md" />}
                                            </button>
                                        );
                                    })}
                                </div>
                        ...
                        <section>
                            <h2 className="text-lg font-extrabold text-[#2A2340] mb-6 tracking-tight flex items-center gap-3">
                                <span className="w-8 h-8 rounded-md bg-[#F0F4FF] text-[#5C89FF] flex items-center justify-center text-sm">2</span>
                                Available Time
                            </h2>
                            <div className="grid grid-cols-3 gap-3">
                                {SLOTS.map((slot) => (
                                    <button
                                        ...
                                        className={`py-4 rounded-md text-[11px] font-extrabold transition-all border
                                            ${selectedSlot === slot
                    ...
                    <div className="space-y-10">
                        <section>
                            <h2 className="text-lg font-extrabold text-[#2A2340] mb-6 tracking-tight flex items-center gap-3">
                                <span className="w-8 h-8 rounded-md bg-[#EAF6EE] text-[#5C9B73] flex items-center justify-center text-sm">3</span>
                                Appointment Details
                            </h2>

                            <div className="bg-white rounded-[40px] p-8 shadow-sm border border-[#F2E9E9] space-y-6">
                                {/* Doctor Card Mini */}
                                <div className="flex items-center gap-4 p-4 bg-[#FDF8F8] rounded-md border border-[#F2E9E9] mb-4">
                                    <img src={doctor.image} alt={doctor.name} className="w-14 h-14 rounded-md object-cover shadow-sm" />
                                    <div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-[#FAFAFA] rounded-md py-4 pl-12 pr-4 outline-none border border-[#F2E9E9] font-bold text-sm text-[#2A2340] focus:ring-4 focus:ring-[#FEE7EC] transition-all"
                                        />
                                    </div>
                                </div>
                                    ...
                                    <div className="flex gap-3">
                                        {TYPES.map(type => (
                                            <button
                                                ...
                                                className={`flex-1 py-4 rounded-md text-[10px] font-extrabold transition-all border
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <textarea
                                            ...
                                            rows="3"
                                            className="w-full bg-[#FAFAFA] rounded-md py-4 pl-12 pr-4 outline-none border border-[#F2E9E9] font-bold text-sm text-[#2A2340] focus:ring-4 focus:ring-[#FEE7EC] transition-all resize-none"
                                        />
                                    </div>
                                </div>

                                {error && (
                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 p-4 bg-red-50 rounded-md text-red-500 text-[10px] font-bold">
                                        <AlertCircle size={14} />
                                        {error}
                                    </motion.div>
                                )}
