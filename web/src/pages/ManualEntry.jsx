...
import { predictionsApi, profilesApi } from '../api';
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const predData = await predictionsApi.createPrediction({
                ...
                blood_pressure: Number(formData.systolic) || 80,
                skin_thickness: 20,
                insulin: Number(formData.insulin) || 80,
                bmi: Number(bmi) || 25,
                diabetes_pedigree_function: formData.familyHistory ? 0.8 : 0.2,
                age: 28,
            });
            ...
            try {
                await profilesApi.updateProfile({
                    pregnancy_week: formData.pregnancyWeek ? Number(formData.pregnancyWeek) : undefined,
                });
            } catch (pErr) {
                console.warn("Profile update optional sync failed:", pErr);
            }
        ...
        <div className="bg-white px-6 pt-12 pb-6 border-b border-gray-50">
            <div className="flex items-center justify-between mb-6">
                <button
                    ...
                    className="p-2.5 bg-white rounded-md shadow-sm border border-gray-100 italic"
                >
            ...
            <div className="h-1.5 w-full bg-pink-50 rounded-md overflow-hidden">
                <motion.div
                    initial={{ width: 0 }}
            ...
            case 8:
                return (
                    <div className="flex flex-col items-center justify-center h-full py-20 text-center">
                        <div className="w-20 h-20 border-4 border-pink-50 border-t-pink-500 rounded-md animate-spin mb-8" />
            ...
            <div className="flex justify-between items-start mb-6">
                ...
                {badge && (
                    <button
                        ...
                        className="px-4 py-2 bg-pink-50 text-pink-500 text-[10px] font-extrabold rounded-md uppercase tracking-wider"
                    >
                        {badge}
                    </button>
                )}
...
function ToggleButton({ label, active, onToggle }) {
    return (
        <button
            onClick={onToggle}
            className={`w-full p-5 rounded-md flex items-center justify-between border transition-all ${active ? 'bg-pink-50 border-pink-100' : 'bg-white border-gray-50'}`}
        >
            <span className="font-bold text-gray-700">{label}</span>
            <div className={`w-12 h-6 rounded-md relative transition-all ${active ? 'bg-pink-500' : 'bg-gray-200'}`}>
                <div className={`absolute top-1 w-4 h-4 bg-white rounded-md transition-all ${active ? 'right-1' : 'left-1'}`} />
            </div>
        </button>
    );
}
...
function InfoCard({ text }) {
    return (
        <div className="p-5 bg-white rounded-md border border-gray-100 shadow-sm flex gap-4">
