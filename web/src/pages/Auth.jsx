...
function SignInScreen({ onGoSignUp, onGoForgot, onSuccess }) {
    ...
    const handle = async (e) => {
        ...
        try {
            ...
            setLoading(false);
            setError(err.response?.data?.error?.message || err.response?.data?.detail || err.message || "Invalid email or password");
        }
    };
...
function CreateAccountScreen({ onGoSignIn, onSuccess }) {
    ...
    const handle = async (e) => {
        ...
        try {
            ...
            setLoading(false);
            setError(err.response?.data?.error?.message || err.response?.data?.detail || err.message || "Failed to create account.");
        }
    };
...
function ResetPasswordScreen({ onBack }) {
    ...
    const handle = async (e) => {
        ...
        try {
            ...
            setLoading(false);
            setSuccess("Password recovery instructions sent! Check your inbox.");
        } catch (err) {
            setLoading(false);
            setError(err.response?.data?.error?.message || err.response?.data?.detail || err.message || "Failed to send reset link.");
        }
    };
