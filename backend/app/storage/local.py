...
class LocalStorageService(StorageService):
    ...
    def _ensure_directories(self) -> None:
        """Ensures internal storage directories exist."""
    ...
    def _resolve_safe_path(self, relative_path: str) -> Path:
        """
        Resolves safe absolute path and prevents directory traversal attacks.
        """
        ...
        # Enforce that full_path starts with root_dir
        if not str(full_path).startswith(str(self.root_dir)):
            raise StorageError(
                message=f"Path traversal detected for path: {relative_path}",
